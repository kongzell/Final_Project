...
- **แดชบอร์ด** ภาพรวมโปรเจค + รายคน (งานเสร็จ ส่งตรงเวลา โดนตีกลับ ภาระงาน) ไว้ประกอบการประเมิน
  มี Timeline (Gantt) วาดจาก est / ลำดับก่อนหลัง / วันเริ่มจริง พร้อมเส้นทางวิกฤต — ไม่ต้องกรอกวันเอง
...
กด **Request changes** ตอนรีวิว PR หรือปิด PR โดยไม่ merge → การ์ดที่รอตรวจถอยกลับ **กำลังทำ** ขึ้นสีแดง "Rework" และอีเมลแจ้งคนรับงาน
(webhook ต้องเลือกส่ง event `Pull request reviews` ด้วย)
...
| GitHub ตีงานกลับ (Request changes / ปิด PR ไม่ merge) | คนที่ถูก assign |
